	...
	return (
		<div className={`config-panel ${disabled ? "is-disabled" : ""}`}>
			...
			<div className="panel config-card">
				...
				<Wheel workers={workers} setWorkers={setWorkers} maxAllowedWorkers={maxAllowedWorkers} />
			</div>

			{/* OCR presets card */}
			<div className="panel config-card">
				<span className="config-card-label">nível de ocr</span>
				<TessModel tessModel={tessModel} setTessModel={setTessModel} />
			</div>
			...
			{ignoredFiles && ignoredFiles.length > 0 && (
				<section
					className="panel warning-card warning-card-panel animate-fade-in"
				...
				>
					...
					<p className="warning-description">
						Os seguintes arquivos foram encontrados no pacote ZIP, mas foram desconsiderados por não
						seguirem a nomenclatura estipulada:
					</p>
